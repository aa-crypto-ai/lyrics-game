...
channel_routing = [
    route("websocket.connect", ws_connect, path=r"^/(?P<room_id>[0-9]+)/$"),
    route("websocket.receive", ws_message, path=r"^/(?P<room_id>[0-9]+)/$"),
    route("websocket.disconnect", ws_disconnect, path=r"^/(?P<room_id>[0-9]+)/$"),
